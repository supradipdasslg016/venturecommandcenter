# --- STREAMLIT CLOUD CHROMADB SQLITE3 MONKEY PATCH ---
try:
...
def run_autonomous_intelligence_generation(topic: str):
    ...
    return {
        ...
        "strategic_positioning": {
            "pestel_summary": "Macro regulatory tracking loops isolate changing compliance conditions for the target landscape. Consumer channels display a structural pivot toward transparency, creating a clear operational white-space for high-utility offerings.",
            "vrio_rows": [
        ...
        "financial_forecasting": {
            ...
            "failure_scenarios": [
                ...
                "Supply Chain Inflation Shock: Disruptions in core raw components adds an unexpected re-engineering overhead penalty."
            ]
        }
    }

# Connect active global data map access pointer
data = st.session_state.master_data if st.session_state.topic else {}
...
# ==============================================================================
    ...
    with d_col2:
        brief_data = data["market_analysis"]["paragraph_synthesis"] if "market_analysis" in data else "Brief Data Core"
        st.download_button("📄 Download Comprehensive Research Brief (Word .doc)", data=brief_data, file_name=f"VCC_Research_Brief_{active_topic.replace(' ', '_')}.doc", use_container_width=True)
...
# ==============================================================================
    ...
    with tab_journey:
        ...
        st.write("📈 **Journey Visualization Elaboration:** The dramatic drop in user satisfaction values during alternative evaluation highlight critical friction points in checkouts.")
...
# ==============================================================================
    ...
    pest_summary = data.get("strategic_positioning", {}).get("pestel_summary", "Stable Regulatory Alignment Channels Map Matrix.")
    st.info(pest_summary)
...
# ==============================================================================
    ...
    if options_list:
        conj_df = pd.DataFrame({
            ...
            "Calculated Profit Vector": [o["profit"] for o in options_list]
        })
    else:
        conj_df = pd.DataFrame()
...
# ==============================================================================
    ...
    active_selection = options_list[selected_idx] if options_list else {"title": "Alpha", "share": "30%", "profit": "8/10", "pros": "N/A", "cons": "N/A"}
...
# ==============================================================================
    ...
    options_list = data.get("conjoint_options", [])
    if options_list:
        active_selection = options_list[st.session_state.selected_product_index]
    else:
        active_selection = {"title": "Alpha", "prd": "N/A", "mrd": "N/A", "business_case": "N/A"}
...
# ==============================================================================
    ...
    st.markdown("🎯 *10-Year Profit Projections and Automated Strategic Risk Appraisals*")
    st.markdown("---")

    f_raw = data.get("financial_forecasting", {"years_labels": ["Y1", "Y2", "Y3"], "revenue_projection": [100, 200, 300], "cost_projection": [80, 120, 150], "failure_scenarios": []})
    ...
    with col_pm_risk:
        ...
        for scenario in f_raw.get("failure_scenarios", []):
            st.error(scenario)
    ...
    c_dl1, c_dl2 = st.columns(2)
    with c_dl1:
        st.download_button("📊 Export 10-Year Corporate Financial Models (Excel .xlsx)", data=chart_f_df.to_csv(), file_name="VCC_10Year_Financial_Model.xlsx", use_container_width=True)
    with c_dl2:
...
# ==============================================================================
    ...
    options_list = data.get("conjoint_options", [{"title": "Alpha Model Configuration Setup"}])
    active_selection = options_list[st.session_state.selected_product_index] if len(options_list) > st.session_state.selected_product_index else options_list[0]
